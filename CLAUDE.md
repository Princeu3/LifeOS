...
- 2026-06-14 — **Delete-entry capability** — `DELETE /timeline/{event_id}` (removes the event + normalized domain row + R2 object for photos) + every timeline entry expands to a "Delete entry" button. Cleared 5 orphaned test captures (a failed e2e run had exited before its cleanup).
- 2026-06-14 — **Withings connected + two fixes.** Owner connected their Withings dev app (16 weigh-ins backfilled). (1) Withings splits one weigh-in across grpids sharing a `date` → dedupe `body_metrics` by **`measured_at`** (migration `0007`) + merge groups per timestamp in the worker (COALESCE) + summary lists every metric. (2) Events stored the LLM's nested parse (`{notes,nutrition:{...}}`) → capture now stores `flatten()`'d structured; UI `fmtVal` recurses (macros readable, ISO dates formatted) + lifts domain-wrapper keys to rows.
